...
const KategorijePage = ({ params }: nameprop) => {
  const [restoraniKat, setRestoraniKat] = useState([]);
  useEffect(() => {
    const filtered: any = RERSTORANI.filter((restoran) =>
      restoran.vrsta.toLowerCase().includes(params.kategorija.toLowerCase())
    );
    setRestoraniKat(filtered);
  }, [RERSTORANI, params.kategorija]);
  ...
  const toggleMenu = () => {
    setIsOpen(!isOpen);
    console.log(restoraniKat);
  };

  const [searchQuery, setSearchQuery] = useState('');

  const filteredRestorani = restoraniKat.filter((restoran: any) =>
  ...
  return (
    <section className="flex flex-col w-full sm:p-0 p-4 overflow-x-hidden">
      <div className="w-full relative flex-col flex-center  p-2 mb-4 mt-20 sm:mt-0">
        ...
        <h2 className="dark:text-white text-3xl font-semibold p-2 capitalize">
          {params.kategorija === 'fast' ? 'Fast Food' : params.kategorija}
        </h2>
